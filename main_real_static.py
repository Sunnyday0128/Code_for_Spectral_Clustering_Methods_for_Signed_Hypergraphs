...
def log_print(content, log_file="experiment_real_statistic.txt"):
    """同时打印到控制台和日志文件"""
    print(content)  # 控制台输出
    with open(log_file, "a+", encoding="utf-8") as f:
        f.write(content + "\n")  # 文件写入（自动换行）

def main_real_iter(r=0.2):
    iteration = 50
    original_name = 'dataset\\real_dataset\\cora_cocitation.csv'
    ...
    for t in range(iteration):
        ...
        for i in range(len(matrices)):
            ...
            loss_score,conflict_edge = conflict_num.global_new_loss(H,result)
    ...
    for i in range(len(matrices)):
        ...
        # print(cur_score)
        cur_score = [round(j,4) for j in cur_score]
        log_print(f'方法【{names[i]}】的损失分数为{cur_score}')
